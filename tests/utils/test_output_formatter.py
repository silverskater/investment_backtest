...
from backtest.utils.output_formatter import (
    format_and_output_results,
    _display_results_on_console,  # pylint: disable=protected-access
    _save_results_to_file  # pylint: disable=protected-access
)
...
class TestOutputFormatter:
    ...
    def common_params(self, sample_metrics):
        return {
            ...
            "yearly_display_returns": [{"year": 2020, "return": 0.1}, {"year": 2021, "return": 0.2}],
            "portfolio_history": [{"date": "2020", "stocks": [], "action": "initial"}]
        }
    ...
    @patch('click.echo')
    def test_save_results_to_file_csv(self, mock_echo, mock_to_csv, mock_file_open, tmp_path, common_params):
    ...
    @patch('json.dump')
    @patch('click.echo')
    def test_save_results_to_file_json(self, mock_echo, mock_json_dump, mock_file_open, tmp_path, common_params):
    ...
    @patch('click.echo')
    def test_save_results_to_file_json_io_error(self, mock_echo, mock_file_open_raises_ioerror, tmp_path, common_params):
        output_path = str(tmp_path / "results.json")
    ...
    @patch('click.echo')
    def test_save_results_to_file_csv_io_error(self, mock_echo, mock_to_csv_raises_ioerror, tmp_path, common_params):
        output_path = str(tmp_path / "results.csv")
