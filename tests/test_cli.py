...
These tests cover the main CLI commands and options, ensuring the application
...
class TestCli:
    ...
    def test_basic_backtest(self, generic_data_file_factory, monkeypatch):
        """Tests basic backtest functionality with default parameters."""
    ...
    def test_custom_parameters(self, generic_data_file_factory, monkeypatch):
        """Tests backtest execution with custom strategy parameters."""
        monkeypatch.setattr('backtest.cli.validate_strategy', lambda name: True)
        monkeypatch.setattr(
            'backtest.cli.get_strategy_description',
            lambda name: "Custom Test Strategy"
        )
    ...
    def test_output_file(self, generic_data_file_factory, monkeypatch):
        """Tests saving backtest results to JSON and CSV output files."""
        strategy = "exp_fund"
        monkeypatch.setattr('backtest.cli.validate_strategy', lambda name: True)
        monkeypatch.setattr(
            'backtest.cli.get_strategy_description',
            lambda name: "Output Test Strategy"
        )
    ...
    def test_invalid_strategy(self, generic_data_file_factory, monkeypatch):
        """Tests CLI behavior when a non-existent strategy name is provided."""
        monkeypatch.setattr('backtest.cli.validate_strategy', lambda name: False)
        monkeypatch.setattr(
            'backtest.cli.list_available_strategies',
            lambda: [{
                "name": "exp_fund",
                "description": "Investment Strategy: The Exponential Fund (Mocked Error Path)"
            }]
        )
    ...
    def test_missing_arguments(self):
        """Tests the 'run' command when essential arguments are missing."""
        runner = CliRunner()
    ...
    @pytest.mark.parametrize(
        ("test_id, strategy_to_run, cli_options, expected_exit_code, "
         "expected_output_snippets, data_strategy_for_fixture"),
        [
            ("dgi_basic_options", "dgi",
             ["--start-year", "2021", "--end-year", "2021", "--top-n", "3"], 0,
             ["Period: 2021-2021"], "dgi"),
            ("valid_single_year", "dgi",
             ["--start-year", "2021", "--end-year", "2021"], 0,
             ["Period: 2021-2021"], "dgi"),
            ("start_year_equals_end_year", "dgi",
             ["--start-year", "2022", "--end-year", "2022"], 0,
             ["Period: 2022-2022", "Sharpe Ratio: 0.00"], "dgi"),
            ("start_year_greater_than_end_year", "dgi",
             ["--start-year", "2022", "--end-year", "2020"], 1,
             ["Error: start_year cannot be greater than end_year"], "dgi"),
            ("growth_threshold_zero", "exp_fund", ["--growth-threshold", "0.0"], 0,
             ["Backtest Results:"], "exp_fund"),
            ("growth_threshold_high", "exp_fund", ["--growth-threshold", "5.0"], 0,
             ["Warning: Strategy selected no stocks"], "exp_fund"),
            ("top_n_one", "exp_fund", ["--top-n", "1"], 0,
             ["Backtest Results:"], "exp_fund"),
            ("top_n_high", "exp_fund", ["--top-n", "100"], 0,
             ["Backtest Results:"], "exp_fund"),
            ("ps_threshold_zero_with_overlay", "exp_fund",
             ["--risk-overlay", "--ps-threshold", "0.0"], 0,
             ["Backtest Results:"], "exp_fund"),
            ("ps_threshold_high_with_overlay", "exp_fund",
             ["--risk-overlay", "--ps-threshold", "100.0"], 0,
             ["Backtest Results:"], "exp_fund"),
            ("transaction_cost_zero", "value_play", ["--transaction-cost", "0.0"], 0,
             ["Total Transaction Costs (as % of avg rebalanced value): 0.00%"], "value_play"),
            ("transaction_cost_mid", "value_play", ["--transaction-cost", "0.1"], 0,
             ["Total Transaction Costs (as % of avg rebalanced value):"], "value_play"),
            ("transaction_cost_high", "value_play", ["--transaction-cost", "1.0"], 0,
             ["Total Transaction Costs (as % of avg rebalanced value):"], "value_play"),
            ("transaction_cost_invalid_too_high", "value_play", ["--transaction-cost", "1.1"], 2,
             ["Invalid value for '--transaction-cost' / '-tc'"], "value_play"),
            ("hybrid_weighting_flag", "value_play", ["--hybrid-weighting"], 0,
             ["Backtest Results:"], "value_play"),
            ("risk_overlay_flag", "value_play", ["--risk-overlay"], 0,
             ["Backtest Results:"], "value_play"),
    ...
    def test_run_command_cli_options(
        self, test_id, strategy_to_run, cli_options, expected_exit_code,
        expected_output_snippets, data_strategy_for_fixture,
        generic_data_file_factory, monkeypatch
    ):
        """Tests the 'backtest run' command with various CLI options."""
        monkeypatch.setattr("backtest.cli.validate_strategy", lambda name: True)
        monkeypatch.setattr(
            "backtest.cli.get_strategy_description",
            lambda name: f"Mocked {name} Strategy"
        )
